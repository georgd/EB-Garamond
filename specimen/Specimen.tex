...
\section{Features}
\subsection{Locals}
\paragraph*{German} prevents f-ligatures that are usually not allowed like fb, fh, fj, ffb, etc. because they would only occur at \enquote{Wortfugen}: {\addfontfeature{Language=German}Laufband, aufjagen, Sauerstoffflasche,…}
...
The feature \texttt{calt} (should be on by default) controls the behaviour of contextually conditioned lettershapes. This allows to have a long-bowed \enquote{f} and \enquote{ſ} like in the original font wherever the bow doesn’t collide with the next letter or cover an empty space: gefährlich fahren, ſüße Weſen;\\
