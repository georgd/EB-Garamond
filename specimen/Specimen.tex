...
\paragraph*{Historical Ligatures} (feature \texttt{hlig}, off by default): historical ligatures for s+t and c+t and for s+p and s+k (the latter two italic only): {\addfontfeature{RawFeature=+hlig}standard, acta, \textit{speak, skies, standard, select}}
\paragraph*{Contextual Ligatures} (feature \texttt{clig}, off by default, italic only): ligatures for final s after a, e, i and u: \textit{\addfontfeature{RawFeature=+clig}finalis, ligatures, humus, rosas}
...
\scriptsize ABCDEFGHIJKLMNOPQQRSTUVWXYZ}
...
\paragraph*{cv21} Variants for ß: first Variant sets all ß to the ß-shape, second to ss: {weiß {\fontspec[CharacterVariant=21:0]{EBGaramond12-Regular}weiß} {\fontspec[CharacterVariant=21:1]{EBGaramond12-Regular}weiß}}
