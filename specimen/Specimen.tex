\documentclass[pagesize,11.5pt,DIV14]{scrartcl}
\usepackage{fontspec}
...
\paragraph*{Serbian} replaces б by \begin{serbian}б\end{serbian}
...
Additionally in italic: g + descending character: \textit{egg, nagy, gjuha} and final s after a, e, i and u: \textit{finalis, ligatures, humus, rosas}
\paragraph*{Discretionary Ligatures} (feature \texttt{dlig}, off by default): historical ligatures for s+t and c+t: {\addfontfeature{RawFeature=+dlig}standard, acta}
\paragraph*{Historical Ligatures} (feature \texttt{hlig}, off by default, italic only): historical ligatures for s+p and s+k: \textit{\addfontfeature{RawFeature=+hlig}speak, skies}
...
\subsection{Swash}
The italic font will have a full set of swash capitals, accessible via the \texttt{swsh}: \\\textit{\addfontfeature{RawFeature=+swsh}ABCDEFGHIJKLMNOPQRSTUVWXYZ}
...
\paragraph*{ss01:} pointed variants of д, л and љ: {\addfontfeature{RawFeature=+ss01} д, л, љ}
